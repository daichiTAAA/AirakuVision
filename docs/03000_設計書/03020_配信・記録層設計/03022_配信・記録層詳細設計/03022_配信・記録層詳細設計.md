...
- ADLS は別リージョン冗長ストレージ (ZRS) を利用し、MinIO から Azure Data Factory 経由で非同期転送。
- FastAPI Metadata Service はコンテナ化し、業務サービス層と同一 VNet 内で動作。PostgreSQL 互換 RDB をメタデータ格納に使用。
...
5. Azure Data Factory が 15 分毎に MinIO から ADLS へ新規チャンクをコピーし、成功後に MinIO オブジェクトへ移行済みタグを付与。コピー完了情報を FastAPI が取り込み、保持期間を更新。
...
- アクセス制御：MediaMTX 用 IAM ユーザー、Data Factory 用 IAM ユーザー、FastAPI 用読み取りロールに分離。
...
- 担当 API：
  ...
  - `PATCH /v1/recordings/{id}/status`（ADLS コピー完了反映）
  - `GET /v1/recordings/{id}`（業務サービス層からの照会）
  - `POST /v1/stream-events`（プライバシーイベント連携）
- データモデル：PostgreSQL に録画テーブル、統計テーブル、通知キューを保持。
- ジョブ：Celery/スケジューラで ADLS コピー確認、Azure Databricks ジョブ起動、MinIO クリーンアップを自動化。
...
### 7.3 ADLS コピー結果通知 (Data Factory → FastAPI)
- `POST /internal/data-transfer/completed`
- ボディ：録画 ID、コピー件数、転送時間、結果ステータス

### 7.4 監視通知 (Prometheus Alertmanager → FastAPI)
- アラート種別：遅延超過、録画失敗、MinIO 容量超過、Data Factory 失敗
...
## 9. 運用・監視
- Prometheus 指標：ストリーム遅延、セッション数、チャンク書き込み時間、MinIO 容量、Data Factory 成功率。
- Grafana ダッシュボード：ライブ配信状況、録画成功率、ADLS コピー遅延、アラート履歴。
- アラート基準：
